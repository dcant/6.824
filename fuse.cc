        ...
        struct fuse_file_info *fi)
{
    ...
    // Change the above line to "#if 1", and your code goes here
	size_t written;
	if(yfs->write(ino,size,off,buf,written) != yfs_client::OK)
	{
		fuse_reply_err(req,ENOENT);
		return;
	}
        ...
        mode_t mode, struct fuse_entry_param *e)
{
     ...
     */
    yfs_client::inum ino;
    int ret = yfs->create(parent,name,mode,ino,extent_protocol::T_FILE);
    if(ret == yfs_client::OK)
    {
    	struct stat s;
    	getattr(ino,s);
        ...
        mode_t mode)
{
    ...
    // Change the above line to "#if 1", and your code goes here
    yfs_client::inum ino;
    if(yfs->create(parent,name,mode,ino,extent_protocol::T_DIR) == yfs_client::OK)
    {
    	struct stat st;
        if(getattr(ino, st) == yfs_client::OK)
            e.attr = st;
    ...
    else if(yfs->create(parent,name,mode,ino,extent_protocol::T_DIR) == yfs_client::EXIST)
    {
    	fuse_reply_err(req,EEXIST);
    }
    else
